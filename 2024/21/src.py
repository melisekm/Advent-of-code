...
def press_all_btns(to_press: list, keypad: Keypad, position_map: dict):
    main_directions = []
    for next_point in to_press:
        start_pos = keypad.curr_pos
        if hasattr(next_point, 'value'):
            next_point = next_point.value
        next_point_pos = position_map[next_point]
        paths = nx.all_shortest_paths(keypad.graph, start_pos, next_point_pos, weight='weight')
        new_directions = []
        for path in paths:
        ...
        keypad.curr_pos = next_point_pos
...
def recurse(parent_paths, directional_keypad_G, directional_position_map, bot_id, max_id):
    ...
    for parent_path in parent_paths:
        this_paths = press_all_btns(
            parent_path,
            Keypad(directional_keypad_G, directional_position_map['A']),
...
def solve_pt1():
    ...
    for code in tqdm(data):
        ...
        main_directions_all_paths = press_all_btns(
            code, Keypad(numeric_keypad_G, numeric_position_map['A']), numeric_position_map
        )
